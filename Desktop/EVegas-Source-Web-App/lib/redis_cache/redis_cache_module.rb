module RedisCacheModule
  ...
  def cache_data_coupon(user_id, clazz_filter_params)
    redis_key_list = []
    ...
    return 'coupon-customer_id_' +  user_id.to_s + Digest::SHA2.hexdigest(redis_key_list.map(&:inspect).join('-'))
   
  end

  def set_key_coupon(key, value)
    begin
      $redis.set(key, value)
    rescue StandardError => e
      logger.error e
  ...
  def redis_cache_get_coupon(key)
    value = nil
    begin
      value = $redis.get(key)
    rescue StandardError => e
  ...
  def redis_cache_delete_by_coupon(key)
    begin
      $redis.del($redis.keys(key))
    rescue StandardError => e
      logger.error e
