class RoulettesController < ApplicationController
  ...
  def index
    ...
    if is_blank(@search_query)
      @roulettes = Roulette.order('id desc').paginate(page: @page, :per_page => PAGING_CONFIG[:LIMIT])
    else
      @search_query = @search_query.strip
      @roulettes = Roulette.where("name LIKE ?", "%#{@search_query}%").order('id desc').paginate(page: @page, :per_page => PAGING_CONFIG[:LIMIT])
    end
  end
