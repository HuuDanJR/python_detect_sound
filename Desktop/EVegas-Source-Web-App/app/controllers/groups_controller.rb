class GroupsController < ApplicationController
  ...
  def edit_role
    ...
    @roles = Role.all
    @roles.each do |item|
      resources.push(item[:resource].to_s)
    end
    
    resources = resources.uniq

    @group.roles.each do |item|
      @group.role_ids.push(item.id)
    end
      ...
      @roles.each do |role|
        if item == role[:resource]
          role_item = {
            ...
            checked:  @group.role_ids.include?(role[:id]) 
          }
          role_resource[:role_list].push(role_item)
        end
      end
      
      @role_list.push(role_resource)
    end
  end
