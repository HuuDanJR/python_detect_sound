...
class MyAPIService {
  ...
  Future runDevice({index, position}) async {
    final myString = MyString.GET_DEVICE_API2(
  ...
  Future runDeviceFullURL({required url}) async {
    ...
    print('runDeviceFullURL response data:  ${response.data}');
    return response.data;
  }
  Future runDeviceFullURL2({required url}) async {
    final myString = MyString.GET_DEVICE_API_FULLURL(url);
    print('url full 2:${MyString.GET_DEVICE_API_FULLURL(url)}');
    ...
    print('runDeviceFullURL2 ${response.data}');
    return response.data;
  }
  ...
  Future<dynamic> updateVolumeValue({value, id}) async {
    ...
    print('updateVolumeValue: ${response.data['data']['currentValue']} ${response.data['data']['_id']}');
    return (response.data);
  }

  //update preset'volume data as current
  Future<dynamic> updatePresetVolumeAsCurrent({id}) async {
    final response = await dio.get(
      MyString.UPDATE_PRESET_VOLUME_AS_CURRENT(endpoint: 'preset_update_volume_value', id: id),
      options: Options(
        contentType: Headers.jsonContentType,
        headers: {
          'Content-type': 'application/json; charset=UTF-8',
    ...
    print('updatePresetVolumeAsCurrent: ${response.data}');
    return (response.data);
  }
